...
// create a btree with maxcount=10000, bytemaxrange=4000
// then insert 10000 elements
// Do not have a conceptual understanding of what byte array bkey range
// represents...
...
do_btree_test(struct client *cli)
{
  ...
  {
    char max_range[16];
    sprintf(max_range, "%d", 4000); // "4000". Unclear what this range means
    rc = memcached_coll_attrs_set_maxbkeyrange_by_byte(&coll_attr,
      (unsigned char*)max_range, strlen(max_range));
    assert(rc == MEMCACHED_SUCCESS);
  }
...
torture_btree_bytemaxbkeyrange_init(void)
{
