...
static int
create_key(struct client *cli, const char *key, int base)
{
  ...
  uint8_t *val_ptr;
  int val_len;
  int ok, keylen = strlen(key);
  
  // Create a btree item
  if (0 != client_before_request(cli))
    return -1;
  
  memcached_coll_create_attrs_init(&attr, 20 /* flags */, 1000 /* exptime */,
    10 /* maxcount */);
  ...
  // Insert 4 elements
  for (bkey = base; bkey < base + 4; bkey++) {
    if (0 != client_before_request(cli))
      return -1;
...
do_btree_test(struct client *cli)
{
  ...
  rc = memcached_get_attrs(cli->next_mc, key, keylen, &attr);
  if (0 != client_after_request(cli, 1 /* it's okay... */))
    return -1;
  if (rc == MEMCACHED_NOTFOUND) {
    // If the key does not exist, create a 4-element btree key.
    create_key(cli, key, base);
  }
  else {
    ok = (rc == MEMCACHED_SUCCESS);
    if (!ok) {
      print_log("bop get attrs failed. id=%d key=%s rc=%d(%s)", cli->id, key,
        rc, memcached_strerror(NULL, rc));
    }
  }
  ...
  memcached_coll_attrs_set_expiretime(&attr, 200);
  rc = memcached_set_attrs(cli->next_mc, key, keylen, &attr);
  ok = (rc == MEMCACHED_SUCCESS);
  if (!ok) {
...
torture_btree_exptime_init(void)
{
