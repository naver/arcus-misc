...
do_btree_test(struct client *cli)
{
  ...
  // Insert 4000 elements and delete them all
  ...
  for (bkey = base; bkey < base + 4000; bkey++) {
    ...
    rc = memcached_bop_delete(cli->next_mc, key, keylen,
      bkey, NULL /* filter */, true /* drop_if_empty */);
    ok = (rc == MEMCACHED_SUCCESS);
    if (!ok) {
      print_log("bop delete failed. id=%d key=%s bkey=%llu rc=%d(%s)",
        cli->id, key, (long long unsigned)bkey,
        rc, memcached_strerror(NULL, rc));
    }
    if (0 != client_after_request(cli, ok))
...
torture_btree_ins_del_init(void)
{
