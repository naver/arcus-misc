...
do_btree_test(struct client *cli)
{
  ...
  for (bkey = base; bkey < base + 4000; bkey++) {
    ...
    val_len = sprintf(val_buf, "%d", (int)(bkey+1));
  ...
  for (bkey = base; bkey < base + 4000; bkey++) {
    ...
    rc = memcached_bop_decr(cli->next_mc, key, keylen,
      bkey, (int)(bkey+1), &new_val);
    ok = (rc == MEMCACHED_SUCCESS);
    if (!ok) {
      print_log("bop decr failed. id=%d key=%s bkey=%llu rc=%d(%s)",
        cli->id, key, (long long unsigned)bkey,
        rc, memcached_strerror(NULL, rc));
    }
    else if (new_val != 0) {
      print_log("Unexpected value from bop decr. result=%llu",
  ...
  for (bkey = base; bkey < base + 4000; bkey++) {
    ...
    rc = memcached_bop_incr(cli->next_mc, key, keylen,
      bkey, (int)(bkey+1), &new_val);
    ok = (rc == MEMCACHED_SUCCESS);
    if (!ok) {
      print_log("bop incr failed. id=%d key=%s bkey=%llu rc=%d(%s)",
        cli->id, key, (long long unsigned)bkey,
        rc, memcached_strerror(NULL, rc));
    }
    else if (new_val != (uint64_t)((int)(bkey+1))) {
      print_log("Unexpected value from bop incr. result=%llu",
        (long long unsigned)new_val);
      ok = 0;
    }
    if (0 != client_after_request(cli, ok))
...
torture_btree_decinc_init(void)
{
