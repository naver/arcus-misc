...
public:
    test_client()
    {
        ...
        EXPECT_TRUE(init("localhost:2181", "test-1,test-2,test-3", 4, 16, 500));
...
TEST(ArcusTest, test_Replication_Case1) {
    ...
    bool available = client.get(client.key, FIELD_MIN, FIELD_MAX, &hint, &result, &rc);
...
TEST(ArcusTest, test_Replication_Case2) {
    ...
    bool available = client.get(client.key, FIELD_MIN, FIELD_MAX, NULL, &result, &rc);
